  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90">
        <circle cx={cx} cy={cx} r={r} fill="none" stroke="var(--color-rule)" strokeWidth={size * 0.063} />
...
function FeedbackRow({ scanId, lang }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <button
        onClick={() => submit('up')}
        className={`p-2 rounded-xl border transition-all ${feedback === 'up' ? 'bg-brand/20 border-brand/40 text-brand' : 'border-rule text-ink-3 hover:text-ink hover:border-rule'}`}
      >
    ...
    <div className="flex flex-col min-h-screen animate-fade-up pb-24 px-4 md:px-8 py-6 max-w-4xl mx-auto w-full">
      ...
      {(r.cookingWarning || r.personalizedWarning) && (
        <div className="flex flex-col gap-3 mb-6">
          {r.cookingWarning && (
            <div className="flex items-start gap-3 p-4 rounded-2xl bg-chili/10 border border-chili/30 text-chili text-sm">
                ...
                {r.fssaiCitations.map((c, i) => (
                  <div key={i} className={`flex gap-3 pb-3 ${i < r.fssaiCitations.length - 1 ? 'border-b border-rule' : ''}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-2 mt-1.5 text-[10px] text-ink-3">
                        {c.state && (
              ...
              <div className="rounded-[24px] border border-rule p-5 flex flex-col gap-4"
                ...
                {r.nutrition.calories !== undefined && (
                  <div className="flex justify-between items-center pb-3 border-b border-rule">
                    ...
                    <span className="text-lg font-sans font-bold text-ink">{r.nutrition.calories}
                      <span className="text-xs text-ink-3 ml-1">kcal</span>
                    </span>
                  </div>
                )}
              ...
              <div className="rounded-[24px] border border-rule overflow-hidden divide-y divide-white/5"
                ...
                {validHomeTests.map((test, i) => (
                  <div key={i} className="p-4 md:p-5">
                    ...
                    <p className="text-xs text-ink-2 leading-relaxed mb-3">{test.steps}</p>
                ...
                <ul className="space-y-3">
                  {r.buyingTips.map((tip, i) => (
                    <li key={i} className="flex gap-3 text-xs text-ink-2 leading-relaxed">
                      <span className="text-brand shrink-0 mt-0.5 font-bold">→</span>
                      <span>{tip}</span>
                    </li>
                  ))}
                </ul>
      ...
      {r.verdict && (
        <div className="mt-6 w-full relative overflow-hidden rounded-[24px] border border-surface-200 shadow-2xl p-6"
          ...
          <div className="relative z-10 flex items-start gap-4">
            ...
            <p className="text-sm text-ink leading-relaxed md:pt-2">{r.verdict}</p>
          </div>
        </div>
      )}
            ...
            <div className="rounded-[24px] border border-rule p-6 lg:p-8 shadow-xl bg-surface-100/50">
              <div className="flex flex-col md:flex-row gap-8 items-center md:items-start mb-8">
                ...
                <div className="text-center md:text-left">
                  ...
                  <p className="text-sm text-ink-2 leading-relaxed max-w-lg">{desc}</p>
                </div>
              </div>
