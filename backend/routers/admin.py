...
from app.db.database import MongoSession, get_db
from app.db.database import db as mongo_db
from models.models import COMMUNITY_REPORTS, FSSAI_VIOLATIONS, SCAN_RECORDS, USERS
...
async def get_admin_stats(db_sess: MongoSession = Depends(get_db)):
    ...
    today_scans = await db_sess.count(
        SCAN_RECORDS, {"created_at": {"$gte": today_start}}
    )
    high_risk = await db_sess.count(
        SCAN_RECORDS, {"risk_level": {"$in": ["HIGH", "CRITICAL"]}}
    )
    active_users_rows = await db_sess.aggregate(
        SCAN_RECORDS,
        [
    ...
    avg_rows = await db_sess.aggregate(
        SCAN_RECORDS,
        [
    ...
    # Top food
    top_food_rows = await db_sess.aggregate(
        SCAN_RECORDS,
        [
    ...
    top_food = (
        top_food_rows[0]["_id"]
        if top_food_rows and top_food_rows[0].get("_id")
        else "—"
    )

    # Top city
    top_city_rows = await db_sess.aggregate(
        SCAN_RECORDS,
        [
            {"$match": {"city": {"$ne": None}}},
    ...
    top_city = (
        top_city_rows[0]["_id"]
        if top_city_rows and top_city_rows[0].get("_id")
        else "—"
    )
...
async def get_ml_status():
    ...
    try:
        ...
        status["yolov8"] = {
            "loaded": True,
            "label": "Food Detection (Gemini Vision)",
            "classes": 10,
        }
    except Exception:
    ...
    try:
        ...
        status["indicbert"] = {
            "loaded": True,
            "label": "IndicBERT / MuRIL NLP (Gemini)",
            "mappings": 48,
        }
    except Exception:
...
async def get_scraper_stats(db_sess: MongoSession = Depends(get_db)):
    ...
    last_scrape = (
        last_rows[0]["created_at"].isoformat()
        if last_rows and last_rows[0].get("created_at")
        else None
    )
    ...
    top_state_rows = await db_sess.aggregate(
        FSSAI_VIOLATIONS,
        [
    ...
    top_product_rows = await db_sess.aggregate(
        FSSAI_VIOLATIONS,
        [
